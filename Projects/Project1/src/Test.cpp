...
int main(){
	...
	//HydographicNetwork::Delivery del = hn.GetDeliveryPath(a1, orders, 75, 50, 1);
	...
	std::unordered_map<uint, std::vector<Order>> orders2;
	...
	std::vector<Order> ordersFrom5; ordersFrom5.push_back(Order(100., 0.));
	std::vector<Order> ordersFrom2; ordersFrom2.push_back(Order(1000., 0.));

	orders2.insert(std::make_pair(a5, std::move(ordersFrom5)));
	orders2.insert(std::make_pair(a2, std::move(ordersFrom2)));

	HydrographicNetwork::Delivery del2 = hn.GetDeliveryItinerary(a1, orders2, 1000, 50, 1);
