...
public:  
	...
	void bellmanFordShortestPath(const V &s) const;
	void dijkstraShortestPath(const V &s);
private:
    void resetIndegree();
...
template <class V, class E>
void Graph<V, E>::dijkstraShortestPath( const V &s )
{
	auto elem = std::find_if(_vertices.begin(), _vertices.end(), [&s] (Vertex* ver) { return ver->info == s; });
	if (elem == _vertices.end()) return;

	for (Vertex* ver : _vertices)
	{
		ver->path = NULL;
		ver->dist = std::numeric_limits<int>::max();
		ver->visited = false;
		ver->removed = false;
	}
	...
	std::make_heap(myVertices.begin(), myVertices.end() [] (Vertex* arg0, Vertex* arg1) { return arg0->dist < arg1->dist; });

	while (!myVertices.empty())
	{
		Vertex* u = myVertices[0];
		std::pop_heap(myVertices.begin(), myVertices.end()); myVertices.pop_back();

		for (Edge& edge : u->adj)
		{
			Vertex* w = edge.dest;
			if (v->dist + edge.weight < w->dist)
			{
...
template <class V, class E>
void Graph<V, E>::bellmanFordShortestPath( const V &s ) const
{
	auto elem = std::find_if(_vertices.begin(), _vertices.end(), [&s] (Vertex* ver) { return ver->info == s; });
	if (elem == _vertices.end()) return;
	...
	for (size_t i = 0; i < _vertices.size(); i++) {
		for (const auto& u : _vertices) {
			for (auto& edge : u->adj) {
				if ((u->dist + edge.weight) < edge.dest->dist)
				{
					edge.dest->dist = u->dist + edge.weight;
					edge.dest->path = u;
				}
			}
		}
